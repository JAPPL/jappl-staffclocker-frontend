...
} from './Toaster.svelte_svelte_type_style_lang.js';
...
const ToastIcon = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `${
		typeof icon === 'string'
			? `<div class="animated svelte-1kgeier">${escape(icon)}</div>`
			: `${
					typeof icon !== 'undefined'
						...
						: `${
								type !== 'blank'
									? `<div class="indicator svelte-1kgeier">${validate_component(
											LoaderIcon,
											'LoaderIcon'
									  ).$$render($$result, Object.assign({}, iconTheme), {}, {})}
		${
			type !== 'loading'
				? `<div class="status svelte-1kgeier">${
						type === 'error'
							? `${validate_component(ErrorIcon, 'ErrorIcon').$$render(
									$$result,
									Object.assign({}, iconTheme),
									{},
									{}
							  )}`
							: `${validate_component(CheckmarkIcon, 'CheckmarkIcon').$$render(
									$$result,
									Object.assign({}, iconTheme),
									{},
									{}
...
const ToastMessage = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<div${spread([{ class: 'message' }, escape_object(toast2.ariaProps)], {
		classes: 'svelte-1nauejd'
	})}>${
		typeof toast2.message === 'string'
			? `${escape(toast2.message)}`
			: `${validate_component(toast2.message || missing_component, 'svelte:component').$$render(
					$$result,
					{ toast: toast2 },
					{},
					{}
...
const ToastBar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<div class="${
		'base ' +
		escape(toast2.height ? animation : 'transparent', true) +
		' ' +
		escape(toast2.className || '', true) +
		' svelte-ug60r4'
	}"${add_styles(
		merge_ssr_styles(escape(style, true) + '; ' + escape(toast2.style, true), {
			'--factor': factor
		})
	)}>${
		Component
			? `${validate_component(Component || missing_component, 'svelte:component').$$render(
					...
					{
						message: () => {
							return `${validate_component(ToastMessage, 'ToastMessage').$$render(
								$$result,
								{ toast: toast2, slot: 'message' },
								{},
								{}
							)}`;
						},
						icon: () => {
							return `${validate_component(ToastIcon, 'ToastIcon').$$render(
								$$result,
								{ toast: toast2, slot: 'icon' },
								{},
								{}
							)}`;
						}
			...
			: `${
					slots.default
						? slots.default({ ToastIcon, ToastMessage, toast: toast2 })
						: `
...
const ToastWrapper = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	justifyContent =
		...
		(toast2.position?.includes('right') && 'flex-end') ||
		null;
	return `<div class="${[
	...
	})}${add_attribute('this', wrapperEl, 0)}>${
		toast2.type === 'custom'
			? `${validate_component(ToastMessage, 'ToastMessage').$$render(
					$$result,
					{ toast: toast2 },
					{},
					{}
			...
			: `${
					slots.default
						? slots.default({ toast: toast2 })
						: `
			${validate_component(ToastBar, 'ToastBar').$$render(
				$$result,
				{ toast: toast2, position: toast2.position },
				{},
				{}
			)}
		`
...
const Toaster = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	if (
		$$props.containerClassName === void 0 &&
		$$bindings.containerClassName &&
	...
	$$unsubscribe_toasts();
	return `<div class="${
		'toaster ' + escape(containerClassName || '', true) + ' svelte-1phplh9'
	}"${add_attribute('style', containerStyle, 0)}>${each(_toasts, (toast2) => {
