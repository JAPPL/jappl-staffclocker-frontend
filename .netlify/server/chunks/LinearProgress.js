...
import { f as forwardEventsBuilder, c as classMap } from './Ripple.js';
const LinearProgress = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<div${spread(
		...
		element,
		0
	)}><div class="mdc-linear-progress__buffer"><div class="mdc-linear-progress__buffer-bar"${add_attribute(
		'style',
		Object.entries(bufferBarStyles)
			.map(([name, value]) => `${name}: ${value};`)
			.join(' '),
		0
	)}></div>
    <div class="mdc-linear-progress__buffer-dots"></div></div>
  <div class="mdc-linear-progress__bar mdc-linear-progress__primary-bar"${add_attribute(
		'style',
		Object.entries(primaryBarStyles)
			.map(([name, value]) => `${name}: ${value};`)
			.join(' '),
		0
